...
fn input(input: &Input, input_id: Option<u32>) -> (Vec2, bool, bool, bool) {
    ...
    if let Some(input_id) = input_id {
        ...
        // TODO: divider depends on mouse speed. needs to be chosen so that moving the mouse reasonably fast equals 1
...
impl<'a> specs::System<'a> for Control {
    ...
    fn run(&mut self, mut data: ControlData) {
		...
		for (controlled, spatial, inertial, shooter, bounding) in (&data.controlled, &data.spatial, &mut data.inertial, &mut data.shooter, &data.bounding).join() {
